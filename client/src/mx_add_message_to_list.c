...
void mx_add_message_to_list(chat_window_t* window, const char *sender_name, const char *message_text, const char *sending_time, gboolean is_your_message) {
	...
	gtk_container_add_with_properties(GTK_CONTAINER(msg_main_box), GTK_WIDGET(row), "expand", TRUE, "fill", TRUE, NULL);
	gtk_list_box_insert(GTK_LIST_BOX(window->msgs_list_box), (GtkWidget*)row, -1);
