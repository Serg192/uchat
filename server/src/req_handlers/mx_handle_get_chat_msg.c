...
static inline int find_last_msg_id(const int chat_id __attribute__((unused))) {
	int result = 0;
...
static inline cJSON* make_message_item(const int own_id, sqlite3_stmt* stmt){
    ...
    char* username = (char *)sqlite3_column_text(stmt, 5);
...
static inline void build_msg_array(int chat_id,
  ...
  //  t_list* list = NULL;
    ...
    while(sqlite3_step(stmt) == SQLITE_ROW){
    	const int this_id __attribute__((unused)) = sqlite3_column_int64(stmt, 0);
